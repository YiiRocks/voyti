...
final class UserRowTest extends TestCase
{
    public function testCreateBuildsBlockToggleLabel(): void
    {
        ...
        $row = UserRow::create($user, new ModuleConfig(), new FakeUrlGenerator(), $this->translator(), false, 999999);

        self::assertSame('voyti.view.block_button', $row->blockToggleLabel);

        $user->setBlockedAt(time());
        $blockedRow = UserRow::create($user, new ModuleConfig(), new FakeUrlGenerator(), $this->translator(), false, 999999);
    ...
    {
        $row = UserRow::create($this->createUser(), new ModuleConfig(), new FakeUrlGenerator(), $this->translator(), false, 999999);
    ...
    public function testCreateDisablesSwitchIdentityForSelf(): void
    {
        ...
        $row = UserRow::create($user, $config, new FakeUrlGenerator(), $this->translator(), false, $user->getIdOrZero());
    ...
    public function testCreateHidesForcePasswordChangeWhenDisabled(): void
    {
        ...
        $row = UserRow::create($this->createUser(), $config, new FakeUrlGenerator(), $this->translator(), false, 999999);
    ...
    public function testCreateHidesSwitchIdentityWhenAlreadySwitched(): void
    {
        ...
        $row = UserRow::create($this->createUser(), $config, new FakeUrlGenerator(), $this->translator(), true, 999999);
