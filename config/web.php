...
return [
    'routes' => [
        Group::create('/voyti')
            ->routes(
                // Security
                Route::get('/login')->name('voyti/login')->action([Controller\SecurityController::class, 'login']),
...
if (class_exists(\YiiRocks\Voyti\Controller\api\v1\AdminController::class)) {
    $config['routes'][] = Group::create('/voyti/api/v1')
        ->routes(
