...
final class RegistrationController
{
    ...
    public function register(ServerRequestInterface $request): ResponseInterface
    {
        if (!$this->config->enableRegistration) {
            return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.disabled'), 'translator' => $this->translator]);
        }
        ...
        return $this->viewRenderer->render('registration/register', [
            'model' => $form,
            'config' => $this->config,
            'errors' => $errors,
        ]);
    ...
    public function confirm(ServerRequestInterface $request, int $id, string $code): ResponseInterface
    {
        ...
        if ($user === null || !$this->config->enableEmailConfirmation) {
            return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.invalid_confirmation_link'), 'translator' => $this->translator]);
        }

        if ($this->accountConfirmationService->run($code, $user, $this->userConfirmationService)) {
            return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.complete'), 'translator' => $this->translator]);
        }

        return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.confirmation_link_invalid'), 'translator' => $this->translator]);
    }

    public function resend(ServerRequestInterface $request): ResponseInterface
    {
        if (!$this->config->enableEmailConfirmation) {
            return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.email_confirmation_disabled'), 'translator' => $this->translator]);
        }
        ...
        if ($request->getMethod() === Method::POST) {
            ...
            if ($result->isValid()) {
                ...
                if ($user !== null && $this->resendConfirmationService->run($user)) {
                    return $this->viewRenderer->render('shared/message', ['title' => $this->translator->translate('voyti.registration.new_confirmation_sent'), 'translator' => $this->translator]);
                }
            }
        }

        return $this->viewRenderer->render('registration/resend', ['model' => $form, 'config' => $this->config]);
    }

    public function connect(ServerRequestInterface $request, string $code): ResponseInterface
    {
        return $this->viewRenderer->render('registration/connect', ['code' => $code, 'config' => $this->config]);
    }
}
