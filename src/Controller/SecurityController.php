...
final class SecurityController
{
    ...
    {
        $provider = $this->requestAttributeString($request, 'provider');
        $clientId = $this->requestAttributeString($request, 'client_id');
        $userAttributes = $this->queryParams($request);
        ...
        if ($request->getMethod() === Method::POST) {
            ...
            if ($result->isValid()) {
                ...
                if ($user === null || !$this->passwordHasher->validate($form->password, $user->getPasswordHash())) {
                    ...
                    $remoteAddr = isset($serverParams['REMOTE_ADDR']) && is_string($serverParams['REMOTE_ADDR'])
                        ? $serverParams['REMOTE_ADDR']
                        : '127.0.0.1';
                    $user->setLastLoginIp($this->config->disableIpLogging ? '127.0.0.1' : $remoteAddr);
                    $user->save();
