    ...
    <div class="mt-3 d-flex gap-2 flex-wrap">
        <?php if (!$user->isConfirmed()): ?>
            <form method="post" action="<?= Html::encode($url->generate('voyti/admin-confirm', ['id' => $user->getId()])) ?>">
                <button type="submit" class="btn btn-success btn-sm"><?= $translator->translate('voyti.view.confirm_button', category: 'voyti') ?></button>
            </form>
        <?php endif; ?>
        <?php if ($user->isBlocked()): ?>
            <form method="post" action="<?= Html::encode($url->generate('voyti/admin-block', ['id' => $user->getId()])) ?>">
                <button type="submit" class="btn btn-warning btn-sm"><?= $translator->translate('voyti.view.unblock_button', category: 'voyti') ?></button>
            </form>
        <?php else: ?>
            <form method="post" action="<?= Html::encode($url->generate('voyti/admin-block', ['id' => $user->getId()])) ?>">
                <button type="submit" class="btn btn-warning btn-sm"><?= $translator->translate('voyti.view.block_button', category: 'voyti') ?></button>
            </form>
        <?php endif; ?>
        <?php if ($config->enablePasswordExpiration): ?>
            <form method="post" action="<?= Html::encode($url->generate('voyti/admin-force-password', ['id' => $user->getId()])) ?>">
                <button type="submit" class="btn btn-outline-secondary btn-sm"><?= $translator->translate('voyti.view.force_password_change_button', category: 'voyti') ?></button>
            </form>
        <?php endif; ?>
        <form method="post" action="<?= Html::encode($url->generate('voyti/admin-delete', ['id' => $user->getId()])) ?>" onsubmit="return confirm('<?= $translator->translate('voyti.view.delete_user_confirm', category: 'voyti') ?>')">
            <button type="submit" class="btn btn-danger btn-sm"><?= $translator->translate('voyti.view.delete_button', category: 'voyti') ?></button>
        </form>
    </div>
</div>
