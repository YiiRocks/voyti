...
$url = $this->get('url');
?>
<?php $this->setTitle($translator->translate('voyti.view.role.title', category: 'voyti')); ?>
<div class="voyti-rbac-index">
    <div class="d-flex justify_content-between align-items-center mb-3">
        <h1><?= $translator->translate('voyti.view.role.title', category: 'voyti') ?></h1>
...
<?php
$form = Html::form(
    $url->generate('voyti/roles-index'),
    'get',
    ['class' => 'mb-3']
);
?>
            ...
            <tbody>
                <?php foreach ($items as $role): ?>
                    <tr>
                        ...
                        <td><?= Html::encode(implode(', ', array_map(fn ($c) => $c->getName(), $role instanceof Role ? [] : []))) ?></td>
                        <td class="text-end">
                ...
                $deleteForm = Html::form(
                    $url->generate('voyti/roles-delete', ['name' => $role->getName()]),
                    'post',
                    ['class' => 'd-inline']
                );
                    ?>
