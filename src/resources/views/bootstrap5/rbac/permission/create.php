...
<div class="voyti-rbac-create">
    <h2 class="mb-4"><?= $translator->translate('voyti.view.permission.create_title') ?></h2>
    <form action="<?= Html::encode($url->generate('voyti/permissions-create')) ?>" method="post" novalidate>
        ...
        <?php endif; ?>
        <?= Text::widget()->name('permission[name]')->label($translator->translate('voyti.view.name_label'))->value($model->name) ?>
        <?= Text::widget()->name('permission[description]')->label($translator->translate('voyti.view.description_label'))->value($model->description) ?>
