...
<div class="voyti-rbac-index">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0"><?= $translator->translate('voyti.view.rule.title') ?></h2>
        <a href="<?= Html::encode($url->generate('voyti/rules-create')) ?>" class="btn btn-primary"><?= $translator->translate('voyti.view.rule.create_link') ?></a>
    </div>
            ...
            <tbody>
                <?php foreach ($rules as $ruleName): ?>
                    <tr>
                        ...
                        <td class="text-end">
                            <a href="<?= Html::encode($url->generate('voyti/rules-update', ['name' => $ruleName])) ?>" class="btn btn-sm btn-outline-secondary"><?= $translator->translate('voyti.view.update_link') ?></a>
