...
echo Html::a($translator->translate('voyti.view.rule.create_link', category: 'voyti'), $url->generate('voyti/admin-rbac-rules-create'))->class('btn', 'btn-primary');
echo Html::div()->close();
...
foreach ($rules as $ruleName) {
    ...
    echo Html::a($translator->translate('voyti.view.update_link', category: 'voyti'), $url->generate('voyti/admin-rbac-rules-update', ['name' => $ruleName]))->class('btn', 'btn-sm', 'btn-outline-secondary', 'me-1');

    echo Html::form()
        ->post($url->generate('voyti/admin-rbac-rules-delete', ['name' => $ruleName]))
        ->csrf($csrf)
        ->class('d-inline')
        ->open();
